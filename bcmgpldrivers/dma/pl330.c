...
#include "../../kernel/linux-3.6.5/drivers/dma/dmaengine.h"
...
pl330_probe(struct platform_device *adev)
{
	...
	pdat =(struct dma_pl330_platdata * )adev->dev.platform_data;
...
//	}
	...
	dev_set_drvdata(&adev->dev, pdmac);
...
//	clk_enable(pdmac->clk);
   ...
   res_irq = platform_get_resource(adev, IORESOURCE_IRQ, 0);
   if (res_irq == NULL) {
	   dev_err(&adev->dev, "no IRQ resource found\n");
	   ret = -ENXIO;
	   goto probe_err2;
   } 
   irq = res_irq->start;
 
    for(i=0; i< resource_size(res_irq); i++)
	{
	    ret = request_irq((irq + i), pl330_irq_handler, 0,
...
static int __devexit pl330_remove(struct platform_device *adev)
{
	struct dma_pl330_dmac *pdmac = dev_get_drvdata(&adev->dev);
