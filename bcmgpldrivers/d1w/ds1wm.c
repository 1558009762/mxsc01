...
#define DS1WM_PRINT if(1) printk
static inline void ds1wm_write_register(struct ds1wm_data *ds1wm_data, u32 reg,
					u8 val)
{
    writel(val, ds1wm_data->map + D1W_DIN_OFFSET);
    writel(reg, ds1wm_data->map + D1W_ADDR_OFFSET);
    writel(0x3, ds1wm_data->map + D1W_CTRL_OFFSET);
}

static inline u8 ds1wm_read_register(struct ds1wm_data *ds1wm_data, u32 reg)
{
    writel(reg, ds1wm_data->map + D1W_ADDR_OFFSET);
    writel(0x2, ds1wm_data->map + D1W_CTRL_OFFSET);
    return readb(ds1wm_data->map + D1W_DOUT_OFFSET);
}
...
void ds1wm_enable(struct ds1wm_data *ds1wm_data)
{
    unsigned int reg_value;

    reg_value = *(unsigned int *)ds1wm_data->clk_gating_reg;
    reg_value |= 1<<5;  /*D1W clock gating disable*/
    *(unsigned int *)ds1wm_data->clk_gating_reg = reg_value;

    reg_value = *(unsigned int *)ds1wm_data->io_mux;
    reg_value &= ~(7<<16);  /*3b'000: D1W  3b'001: UART4, set it as D1W mode*/
    *(unsigned int *)ds1wm_data->io_mux = reg_value;
}

void ds1wm_disable(struct ds1wm_data *ds1wm_data)
{
    unsigned int reg_value;

    reg_value = *(unsigned int *)ds1wm_data->clk_gating_reg;
    reg_value &= ~(1<<5);   /*D1W clock gating enable*/
    *(unsigned int *)ds1wm_data->clk_gating_reg = reg_value;

    reg_value = *(unsigned int *)ds1wm_data->io_mux;
    reg_value |= 1<<16;  /*3b'000: D1W  3b'001: UART4, set it as UART4 mode*/
    *(unsigned int *)ds1wm_data->io_mux = reg_value;
}
			...
			u8 search_type, w1_slave_found_callback slave_found)
{
	...
	while (true) {
		...
		for (i = 0; i < 16; i++) {
			...
			_r_prime = ((resp & 0x02) >> 1) |   /*12345678 ===> 1357*/
			((resp & 0x08) >> 2) |
			((resp & 0x20) >> 3) |
			((resp & 0x80) >> 4);

			_d = ((resp & 0x01) >> 0) |     /*12345678 ===> 2468*/
			((resp & 0x04) >> 1) |
			((resp & 0x10) >> 2) |
			((resp & 0x40) >> 3);
...
static int ds1wm_probe(struct platform_device *pdev)
{
    ...
    DS1WM_PRINT("Direct reg = %08x\n", (unsigned int)ds1wm_data->map);
	...
	ds1wm_data->clk_gating_reg = ioremap(res->start, resource_size(res));
	if (!ds1wm_data->clk_gating_reg) {
		ret = -ENOMEM;
		goto err1;
	}
    DS1WM_PRINT("clk_gating_reg = %08x\n", (unsigned int)ds1wm_data->clk_gating_reg);
	...
	ds1wm_data->io_mux = ioremap(res->start, resource_size(res));
	if (!ds1wm_data->io_mux) {
		ret = -ENOMEM;
		goto err2;
	}
    DS1WM_PRINT("io_mux = %08x\n", (unsigned int)ds1wm_data->io_mux);
    ...
    DS1WM_PRINT("Add d1w master successfully\n");
	return 0;
...
err2:
    iounmap(ds1wm_data->clk_gating_reg);
err1:
...
void platform_ds1wm_release(struct device *pDev)
{
    return;
}
...
static struct platform_device ds1wm_device = {
        ...
        .dev            =       {
                ...
                .release = platform_ds1wm_release
        },
...
{
	printk("DS1WM w1 busmaster driver - (c) 2014 Broadcom\n");
    platform_device_register(&ds1wm_device);
...
static void __exit ds1wm_exit(void)
{
    ...
    platform_device_unregister(&ds1wm_device);
}

late_initcall(ds1wm_init);
module_exit(ds1wm_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Jim(Zhijun) Sun <zhijun.sun@broadcom.com>");
