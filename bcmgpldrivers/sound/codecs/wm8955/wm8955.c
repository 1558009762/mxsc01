...
#define CLKOUT_EN 		(1<<7)	// Clock out enable
...
#if defined(CONFIG_IPROC_GSIO_SPI)	
// Call the cca_spi driver to access the codec
static int WM8955_write( unsigned short regAddr, unsigned short registerValue )
{
    unsigned short command;

    command = (regAddr << 9) | ( 0x1FF & registerValue );

    if ( SPI_ERR_NONE ==cca_spi_write_codec((unsigned char*)&command, 2 ))
    {
        DBG("Command issued, register 0x%x, value 0x%x\n",regAddr, ( 0x1FF & registerValue )  );
        return 0;
    }
    return -1;
}

static int WM8955_hw_params(struct snd_pcm_hw_params *params)
{	
	uint32_t rate;
	uint32_t mclk;
	int i;
	int sr = -1;
	unsigned short sampleRateRegValue, clockingPllRegValue;
	bool found = false;	
	
	DBG("%s\n", __FUNCTION__);
	
	pcm_format = params_format(params);		// Save PCM format for later.
	
	rate = params_rate(params);
	mclk = 12288000;

	/* Find a match for Sample Rate from the table */
	for (i = 0; i < ARRAY_SIZE(clock_cfgs); i++) {
		if (rate != clock_cfgs[i].fs)
			continue;
		sr = clock_cfgs[i].sr;

		if (mclk == clock_cfgs[i].mclk) {
			found = true;
			break;
		}
	}
	...
	if (found != true) {
		printk(KERN_ERR "%s: unsupported audio sample rate %d Hz and mclk %d Hz "
		       "combination\n", __FUNCTION__, rate, mclk);
		return -EINVAL;
	}

	DBG("%s: rete = %d sr = 0x%x\n", __FUNCTION__, rate, sr);
			
	sampleRateRegValue = (sr << 1);	
	sampleRateRegValue |= WM8955_MCLKDIV2;			// Divide by 2 to get 12.288Mhz clock
	clockingPllRegValue = CLKOUT_EN;				// Enable clock out
	
	WM8955_write(WM8955_SAMPLE_RATE, sampleRateRegValue);
	WM8955_write(WM8955_CLOCKING_PLL, clockingPllRegValue);
	
	return 0;
}

static int WM8955_set_fmt(unsigned int fmt)
{
	unsigned short aif = 0;
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		aif = 0;
		DBG("%s: PCM Format S16_LE\n", __FUNCTION__);		
		break;
	case SNDRV_PCM_FORMAT_S20_3LE:
		aif = 0x4;
		DBG("%s: PCM Format S20_3LE\n", __FUNCTION__);		
		break;
	case SNDRV_PCM_FORMAT_S24_LE:
		aif = 0x8;
		DBG("%s: PCM Format S24_LE\n", __FUNCTION__);		
		break;
	case SNDRV_PCM_FORMAT_S32_LE:
		aif = 0xc;
		DBG("%s: PCM Format S32_LE\n", __FUNCTION__);		
		break;
	default:
		return -EINVAL;
	}
	...
	WM8955_write(WM8955_AUDIO_INTERFACE, aif);

	return 0;
}

#elif defined(CONFIG_I2C) || defined(CONFIG_I2C_MODULE)
...
{
	int ret = 0;
	unsigned short pwrManagement2RegValue;
	unsigned short lMixer1RegValue, rMixer1RegValue;
		
	DBG("%s\n", __FUNCTION__);
	
#if defined(CONFIG_IPROC_GSIO_SPI)			
	if (cca_spi_init_codec() == SPI_ERR_NONE)
		DBG("%s: Initialized spi\n", __FUNCTION__);
	else
		DBG("%s: Failed to initialize spi\n", __FUNCTION__);
		
	// Reset the codec
	WM8955_write(WM8955_RESET, 0);
	msleep(100);
				
	// Setup mclk, Codec master clock from MCLK pin, clockout enabled, clockoutfrom MCLK pin
	// Setup sample rate
	//DBG("%s: Enabling clockout\n", __FUNCTION__);
	///sampleRateRegValue = WM8955_MCLKDIV2 | WM8955_SAMPLE_RATE_8KHZ;
	//sampleRateRegValue = WM8955_MCLKDIV2 | WM8955_SAMPLE_RATE_16KHZ;
	//sampleRateRegValue = 0;
	//clockingPllRegValue = CLKOUT_EN;
	//WM8955_write(WM8955_SAMPLE_RATE, sampleRateRegValue);				// Reg 0x8  Set MCLK (24Mhz)
	//WM8955_write(WM8955_CLOCKING_PLL, clockingPllRegValue);				// Reg 0x2B
	
	// Configure audio interface
	// Set Slave mode, I2S format and 16bit word length
	//DBG("%s: Configuring Slave mode audio interface\n", __FUNCTION__);
	///audioInterfaceRegValue = WM8955_I2S_FORMAT;
	///audioInterfaceRegValue = WM8955_I2S_FORMAT | WM8955_BCLKINV;		// Invert BitClock
	///audioInterfaceRegValue = WM8955_I2S_FORMAT | WM8955_LRP;			// Invert LRClock
	///audioInterfaceRegValue = WM8955_I2S_FORMAT | WM8955_BCLKINV | WM8955_LRP;			// Invert BitClock and LRClock

	// Set master mode, I2S format and 16bit word length
	//DBG("%s: Configuring Master mode audio interface\n", __FUNCTION__);
	//audioInterfaceRegValue = WM8955_I2S_FORMAT | WM8955_MS;
	
	//WM8955_write(WM8955_AUDIO_INTERFACE, audioInterfaceRegValue);	// 0x7
		
	// Setup left and right mixers
	DBG("%s: Enabling DACs and Mixers\n", __FUNCTION__);
	lMixer1RegValue = 0x100;
	rMixer1RegValue = 0x100;
	WM8955_write(WM8955_LEFT_OUT_MIX_1, lMixer1RegValue);
	WM8955_write(WM8955_RIGHT_OUT_MIX_2, rMixer1RegValue);
	
	// Enable Left and Right outputs (LOUT2, ROUT2)
	DBG("%s: Enabling outputs\n", __FUNCTION__);
	pwrManagement2RegValue = WM8955_DACL | WM8955_DACR | WM8955_LOUT2 | WM8955_ROUT2 | WM8955_LOUT1 | WM8955_ROUT1;
	WM8955_write(WM8955_POWER_MANAGEMENT_2, pwrManagement2RegValue);
	
	// Enable 50K divider
	DBG("%s: Enabling VMID\n", __FUNCTION__);	
	WM8955_write(WM8955_POWER_MANAGEMENT_1, 0x80);
	
	// Unmute DAC
	DBG("%s: Unmuting DAC\n", __FUNCTION__);
	WM8955_write(WM8955_DAC_CONTROL, 0);
...
#if defined(CONFIG_IPROC_GSIO_SPI)
EXPORT_SYMBOL(WM8955_hw_params);
EXPORT_SYMBOL(WM8955_set_fmt);
#endif
