...
static int WM8750_pcm_hw_params(struct snd_pcm_hw_params *params)
{
	uint32_t rate;
	uint32_t mclk;
	int coeff;
	unsigned short sampleRateRegValue;
	
	pcm_format = params_format(params);		// Save PCM format for later.
	
	rate = params_rate(params);
	mclk = 12288000;						// Clock is 12.288 Mhz on SVK board for WM8750 codec
	
	if ((coeff = get_coeff(mclk, rate)) == EINVAL)
		return EINVAL;

	sampleRateRegValue = coeff_div[coeff].sr << 1;
	
	DBG("%s: rate = %d coeff = %d sr = 0x%x\n", __FUNCTION__, rate, coeff, coeff_div[coeff].sr);
	
	WM8750_write(WM8750_SRATE, sampleRateRegValue);
...
static int WM8750_set_dai_fmt(unsigned int fmt)
{
	...
	switch (pcm_format) {
	case SNDRV_PCM_FORMAT_S16_LE:
		DBG("%s: SNDRV_PCM_FORMAT_S16_LE\n", __FUNCTION__);		
		break;
	case SNDRV_PCM_FORMAT_S20_3LE:
		iface |= 0x0004;
		DBG("%s: SNDRV_PCM_FORMAT_S20_3LE\n", __FUNCTION__);	
		break;
	case SNDRV_PCM_FORMAT_S24_LE:
		iface |= 0x0008;
		DBG("%s: SNDRV_PCM_FORMAT_S24_LE\n", __FUNCTION__);	
		break;
	case SNDRV_PCM_FORMAT_S32_LE:
		DBG("%s: SNDRV_PCM_FORMAT_S32_LE\n", __FUNCTION__);	
		iface |= 0x000c;
		break;
	}
	...
	case SND_SOC_DAIFMT_CBM_CFM:
		iface = 0x0040;
		DBG("%s: SND_SOC_DAIFMT_MASTER\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_CBS_CFS:
		DBG("%s: SND_SOC_DAIFMT_SLAVE\n", __FUNCTION__);	
		break;
	default:
		return -EINVAL;
	}
	...
	case SND_SOC_DAIFMT_I2S:
		iface |= 0x0002;
		DBG("%s: SND_SOC_DAIFMT_FORMAT_I2S\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		DBG("%s: SND_SOC_DAIFMT_FORMAT_RIGHT_J\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		iface |= 0x0001;
		DBG("%s: SND_SOC_DAIFMT_FORMAT_LEFT_J\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_DSP_A:
		iface |= 0x0003;
		DBG("%s: SND_SOC_DAIFMT_FORMAT_DSP_A\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_DSP_B:
		iface |= 0x0013;
		DBG("%s: SND_SOC_DAIFMT_FORMAT_DSP_B\n", __FUNCTION__);	
		break;
	default:
		return -EINVAL;
	}
	...
	switch (fmt & SND_SOC_DAIFMT_INV_MASK) {
	case SND_SOC_DAIFMT_NB_NF:
		DBG("%s: SND_SOC_DAIFMT_NB_NF\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_IB_IF:
		iface |= 0x0090;
		DBG("%s: SND_SOC_DAIFMT_IB_IF\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_IB_NF:
		iface |= 0x0080;
		DBG("%s: SND_SOC_DAIFMT_IB_NF\n", __FUNCTION__);	
		break;
	case SND_SOC_DAIFMT_NB_IF:
		iface |= 0x0010;
		DBG("%s: SND_SOC_DAIFMT_NB_IF\n", __FUNCTION__);	
		break;
	default:
		return -EINVAL;
	}
...
static int WM8750_set_bias_level(int level)
{

	switch (level) {
	case SND_SOC_BIAS_ON:
		DBG("%s: SND_SOC_BIAS_ON\n", __FUNCTION__);	
		/* set vmid to 50k and unmute dac */
		WM8750_write(WM8750_PWR1, 0x00c0);
		break;
	case SND_SOC_BIAS_PREPARE:
		DBG("%s: SND_SOC_BIAS_PREPARE\n", __FUNCTION__);	
		break;
	case SND_SOC_BIAS_STANDBY:
		DBG("%s: SND_SOC_BIAS_STANDBY\n", __FUNCTION__);	
		if (bias_level == SND_SOC_BIAS_OFF) {
			/* Set VMID to 5k */
			WM8750_write(WM8750_PWR1, 0x01c1);
		...
		/* mute dac and set vmid to 500k, enable VREF */
		WM8750_write(WM8750_PWR1, 0x0141);
		break;
	case SND_SOC_BIAS_OFF:
		DBG("%s: SND_SOC_BIAS_OFF\n", __FUNCTION__);	
		WM8750_write(WM8750_PWR1, 0x0001);
		break;
	}
...
{
	unsigned short pwr2_reg, pwr1_reg;
	...
	// Set bias level to standby
	//WM8750_set_bias_level(SND_SOC_BIAS_STANDBY);
...
#if defined(CONFIG_IPROC_GSIO_SPI)
EXPORT_SYMBOL(WM8750_pcm_hw_params);
EXPORT_SYMBOL(WM8750_set_dai_fmt);
EXPORT_SYMBOL(WM8750_set_bias_level);
#endif
