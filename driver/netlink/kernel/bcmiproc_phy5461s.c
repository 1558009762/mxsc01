...
static struct nla_policy doc_exmpl_genl_policy[DOC_EXMPL_A_MAX + 1] = {
     [ATTR_PHY_CFG_DATA] = { .type = NLA_U32 },
};
...
static int phy_reg_write(struct sk_buff *skb, struct genl_info *info)
{
  ...
  // phy5461_wr_reg(phy_cfg->eth_num, phy_cfg->phyaddr, phy_cfg->flags, 
   ...
   printk("### write ### 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x \n",
				phy_cfg->eth_num, phy_cfg->phyaddr, phy_cfg->flags, 
				phy_cfg->reg_bank, phy_cfg->reg_addr, phy_cfg->data);
...
/****************************************************************************/
void packet_generic_netlink_init(void)
{
    int rc;
...
#if 0
    if ((rc = genl_register_family_with_ops(&doc_exmpl_genl_family, &doc_exmpl_genl_ops_echo, ARRAY_SIZE(doc_exmpl_genl_ops_echo))) != 0) {
        printk("genl_register_family_with_ops() failed with error = %d \n", rc);
        return;
    }
#endif
	printk("init huahuan generic netlink \n");
