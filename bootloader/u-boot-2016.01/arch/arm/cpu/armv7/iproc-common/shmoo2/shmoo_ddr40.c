...
STATIC uint32 _get_random(void)
{
    volatile uint32 *m_w = (DDR_SHMOO_PARAM_MEM_PTR + sizeof(soc_ddr_shmoo_param_t));
    volatile uint32 *m_z = (DDR_SHMOO_PARAM_MEM_PTR + sizeof(soc_ddr_shmoo_param_t) + 4);
    ...
    if (!stat) {
        switch(ctlType) {
            ...
            case DDR_CTLR_T1:
                ...
                *(volatile uint32 *)(DDR_SHMOO_PARAM_MEM_PTR + sizeof(soc_ddr_shmoo_param_t)) = 6483;
                *(volatile uint32 *)(DDR_SHMOO_PARAM_MEM_PTR + sizeof(soc_ddr_shmoo_param_t)+4) = 31245;
                if((ci & 0x1) == 0) {
                    if(_check_dram_either(ci)) {
                        SOC_IF_ERROR_RETURN(_soc_ddr40_phy_PVT_ctl(unit, ci, 1, stat));
                        SOC_IF_ERROR_RETURN(_soc_ddr_shmoo_prepare_for_shmoo(unit, ci));
