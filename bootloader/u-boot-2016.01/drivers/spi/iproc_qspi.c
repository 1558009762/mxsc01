...
extern uint32_t iproc_get_apb_clk(void);
...
#define BSPI_STRAP_OVERRIDE_CTRL_MASK       (0x00000001)
/*
                ...
                unsigned int max_hz, unsigned int mode)
{
    ...
    priv = spi_alloc_slave(struct bcmspi_priv, bus, cs);
	if (!priv)
		return NULL;
    ...
    /* Reference clcok is apb_clk * 2 */
    spbr = (iproc_get_apb_clk() * 2) / (2 * priv->max_hz);
                        ...
                        const u8 *tx, u8 *rx, uint bytes, unsigned long flags)
{
    ...
    if (flags & SPI_XFER_BEGIN) {
		...
		priv->bspi_hw->strap_override_ctrl |= cpu_to_le32(BSPI_STRAP_OVERRIDE_CTRL_MASK);
    }
    ...
    if (flags & SPI_XFER_END) {
        bspi_flush_prefetch_buffers(priv);
		/* Switch back to strap setting */
		priv->bspi_hw->strap_override_ctrl &= cpu_to_le32(~BSPI_STRAP_OVERRIDE_CTRL_MASK);
    }
    
    return 0;
}
#ifdef BSPI_EMULATE_RDSR_ENABLED
static int
...
mspi_xfer(struct bcmspi_priv *priv, uint bytes, const u8 *tx, u8 *rx, int end)
{
    ...
    while(bytes) {
        ...
        if (priv->mspi_16bit) {
            ...
            chunk = min(bytes, (uint)(NUM_CDRAM * (priv->mspi_16bit? 2 : 1)));
            queues = (chunk - 1) / 2 + 1;
            bytes -= chunk;
