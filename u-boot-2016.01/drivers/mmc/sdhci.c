...
static void sdhci_set_ios(struct mmc *mmc)
{
    ...
    if (host->quirks & SDHCI_QUIRK_FORCE_DRIVER_STRENGTH) {
		ctrl = sdhci_readb(host, SDHCI_HOST_CONTROL2);
		ctrl &= ~SDHCI_CTRL_DRV_TYPE_MASK;

		if (host->drv_type == SDHCI_DRIVER_TYPE_A) {
			ctrl |= SDHCI_CTRL_DRV_TYPE_A;
		} else if (host->drv_type == SDHCI_DRIVER_TYPE_C) {
			ctrl |= SDHCI_CTRL_DRV_TYPE_C;
		} else if (host->drv_type == SDHCI_DRIVER_TYPE_D) {
			ctrl |= SDHCI_CTRL_DRV_TYPE_D;
		}

		sdhci_writeb(host, ctrl, SDHCI_HOST_CONTROL2);
	}
}
