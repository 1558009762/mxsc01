		     ...
		     struct aead_testvec *template, unsigned int tcount)
{
	...
	for (i = 0, j = 0; i < tcount; i++) {
		if (!template[i].np) {
			...
			if (WARN_ON(template[i].ilen > PAGE_SIZE || template[i].alen > PAGE_SIZE))
				goto out;
            ...
            sg_init_one(&sg[0], input, template[i].ilen + (enc ? authsize : 0) + 16);
            sg_init_one(&asg[0], assoc, template[i].alen);

            aead_request_set_crypt(req, sg, sg, template[i].ilen, iv);
...
static const struct alg_test_desc alg_test_descs[] = {
	{
		...
		.suite = {
			.aead = {
				.enc = {
					.vecs = dtls_hmac_sha1_aes_cbc_enc_tv_template,
					.count = DTLS_HMAC_SHA1_AES_CBC_ENC_TEST_VECTORS
				},
				.dec = {
					.vecs = dtls_hmac_sha1_aes_cbc_dec_tv_template,
					.count = DTLS_HMAC_SHA1_AES_CBC_DEC_TEST_VECTORS
				}
			}
		}
	}, {
