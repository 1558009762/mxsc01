...
static int hack_cmd_ctr;
static int pm_workaround;
#endif
...
void ahci_init_controller(struct ata_host *host)
{
	...
	VPRINTK("HOST_CTL 0x%x\n", tmp);
...
#ifdef BCMINTERNAL
        {
            char *var;
            pm_workaround = 0;
            var = nvram_get("pm_workaround");
            if (var) {
                pm_workaround = simple_strtoul(var, NULL, 0);
            }
            printk("%s: pm_workaround set to 0x%x\n", __FUNCTION__, pm_workaround);
        }
#endif
...
#ifdef BCMINTERNAL
       if ((pm_workaround) && (++hack_cmd_ctr == 63)) {
               u32 tmp;
               pr_debug("restart engine by manipulating PxCMD.ST\n");
#if (0) /* can't use this in IRQ context because it sleeps, and commands
        * can be issued via thread or IRQ.
        */
               ahci_stop_engine(ap);
#else
               tmp = readl(port_mmio + PORT_CMD) & ~PORT_CMD_START;
               writel(tmp, port_mmio + PORT_CMD);
