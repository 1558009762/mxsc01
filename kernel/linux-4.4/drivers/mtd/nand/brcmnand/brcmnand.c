...
static uint8_t brcmnand_read_byte(struct mtd_info *mtd)
{
	...
	case NAND_CMD_RNDOUT:
		...
		/* No byte swap needed for XGS iproc, as 
		 * brcmnand_soc_data_bus_prepare() sets the suitable endian
		 * for fetching data from NAND controller FC to 
		 * ctrl->flash_cache.
		 */
#if defined (CONFIG_MTD_NAND_XGS_IPROC)								
		ret = ctrl->flash_cache[offs >> 2] >> ((offs & 0x03) << 3);		
#else
