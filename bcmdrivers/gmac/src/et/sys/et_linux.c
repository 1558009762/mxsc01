...
iproc_gmac_init_module(void)
{
    ...
    for (idx = 0; idx < IPROC_NUM_GMACS; idx++) {
        ...
        /* Load GMAC driver */
        err = iproc_platform_driver_register(&gmac_pdrv[idx]);
        if (err) {
		    ET_ERROR(("ERROR module_init, could not iproc_platform_driver_register, errno: 0x%08x\n", err));
		    return err;
        } 
        
        gmac_pdev_loaded[idx] = true;
    }
