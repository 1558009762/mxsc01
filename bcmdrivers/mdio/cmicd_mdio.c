...
#define CMICD_BASE_ADDRESS		0x03200000
#define CMC2_OFFSET			0x33000
#define COMMON_OFFSET			0x10000
...
static struct resource cmicd_cmc2_mem = {
	.name	= "cmicd_cmc2",
	.start 	= CMICD_BASE_ADDRESS + CMC2_OFFSET,
	.end 	= CMICD_BASE_ADDRESS + CMC2_OFFSET + 4096 - 1,
	.flags	= IORESOURCE_MEM,
};

#ifdef __BIG_ENDIAN
static struct resource cmicd_common_mem = {
	.name	= "cmicd_common",
	.start 	= CMICD_BASE_ADDRESS + COMMON_OFFSET,
	.end 	= CMICD_BASE_ADDRESS + COMMON_OFFSET + 4096 - 1,
	.flags	= IORESOURCE_MEM,
};
#endif
...
static void __maybe_unused cmicd_mdiobus_test(struct mii_bus *mii_bus)
{
	...
	for (i=0; i<PHY_MAX_ADDR; i++)
	{
		nRet1 = iproc_mii_read(bus_data->logbus_num, bus_data->logbus_type, 
					i, 2, &data1);
		nRet2 = iproc_mii_read(bus_data->logbus_num, bus_data->logbus_type, 
					i, 3, &data2);
		if ((nRet1 < 0) || (nRet2 < 0))
...
static struct cmicd_mdio_ctrl * cmicd_mdio_res_alloc(void)
{
	if (!cmicd_cmc2) {
		...
		cmicd_cmc2->base = ioremap(cmicd_cmc2_mem.start, 
					resource_size(&cmicd_cmc2_mem));
		if (!cmicd_cmc2->base) {
			kfree(cmicd_cmc2);
			cmicd_cmc2 = NULL;
			return NULL;
...
#ifdef __BIG_ENDIAN
		cmicd_cmc2->cmic_common = ioremap(cmicd_common_mem.start,
					resource_size(&cmicd_common_mem));
		if (!cmicd_cmc2->cmic_common) {
			iounmap(cmicd_cmc2->base);
			kfree(cmicd_cmc2);
			cmicd_cmc2 = NULL;
			return NULL;
		}
...
static int __devinit cmicd_mdiobus_probe(struct platform_device *pdev)
{
...
static int __devexit cmicd_mdiobus_remove(struct platform_device *pdev)
{
